...
public class RewardedAdComponent : MonoBehaviour
{
    ...
    public AdEventWithAd OnAdFailedToShow;
    public AdEventString OnImpression;
    public AdEventWithAd OnRewarded;
    ...
    private void HandleAdFailedToShow(object sender, EventArgs args) => OnAdFailedToShow?.Invoke(rewardedAd);

    private void HandleImpression(object sender, ImpressionData impressionData) =>
        OnImpression?.Invoke(impressionData?.rawData ?? string.Empty);

    private void HandleRewarded(object sender, Reward args) => OnRewarded?.Invoke(rewardedAd);
...
// автозагрузка
// апп опен ад чек бокс на колд старт - done
// в шоу дебаг - done 
// вместо эвейк старт - done